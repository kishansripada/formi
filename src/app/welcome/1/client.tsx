               ...
               >
                  <div className="lg:max-w-md max-w-xs flex flex-col  ">
                     <p className="text-3xl font-medium">
                        {"Welcome to FORMI" +
                           (session.user.user_metadata.full_name ? `, ${session.user.user_metadata.full_name.split(" ")[0]}!` : "!")}
                     </p>
                     ...
                     {step === 0 ? (
                                 ...
                                 {[
                                    ...
                                    return (
                                       ...
                                       <button
                                          ...
                                          className="flex flex-row justify-center cursor-default border-2 border-pink-100 border-opacity-40 px-3 py-2 whitespace-nowrap rounded-full text-sm font-medium"
                                       >
                        ...
                        <div className="flex flex-col gap-4 mt-10">
                           <div>
                              <p className="text-xl font-medium mb-1">How did you find out about us?</p>
                              <div className="flex flex-row flex-wrap mb-1 mt-3	 gap-4">
                                 {["TikTok", "A friend told me", "My coach", "My students", "I found it on Google", "Idk I just found it"].map(
                                    (d) => {
                                       return (
                                          // <div>
                                          <button
                                             onClick={() => {
                                                setHowYouFoundOut(d);
                                                setHowYouFoundOutOther("");
                                             }}
                                             style={{
                                                borderColor: howYouFoundOut === d ? "#db2777" : "inherit",
                                             }}
                                             className="flex flex-row justify-center cursor-default border-2 border-pink-100 border-opacity-40 px-3 py-2 whitespace-nowrap rounded-full text-sm font-medium"
                                          >
                                             <p className=" text-center">{d}</p>
                           ...
                           <div className="mb-6">
                              {/* <p className="text-sm font-medium mb-1">Other</p> */}
                              <Input
                                 value={howYouFoundOutOther}
                                 className="bg-transparent"
                                 placeholder="Other..."
                                 onChange={(e) => {
                                    setHowYouFoundOut("");
                                    setHowYouFoundOutOther(e.target.value);
                                 }}
                              ></Input>
                           </div>
                           ...
                           <Button
                              onClick={async () => {
                                 ...
                                 //    response_data: { selectedUses: uses, howYouFoundOut: howYouFoundOutOther || howYouFoundOut },
                                 //    name,
                                 // });
                                 const _ = await supabase
                                    .from("user_data")
